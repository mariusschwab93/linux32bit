...
void main(void)
{
	...
	for(ch=0; ch<MAXCH; ch++) {
		pru1_dram[2*ch  ] = on[ch];		// Copy to DRAM0 so the ARM can change it
		pru1_dram[2*ch+1] = off[ch];	// Interleave the on and off values
		onCount[ch] = on[ch];
		offCount[ch]= off[ch];
	}
	...
	while (1) {
		for(ch=0; ch<MAXCH; ch++) {
			if(onCount[ch]) {
				...
				onCount[ch] = pru1_dram[2*ch];		// Read from DRAM0
				offCount[ch]= pru1_dram[2*ch+1];
			}
		}
	}
}

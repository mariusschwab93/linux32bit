 ...
 * is attached to GPIO 48 which is on P9_15 and the button is attached to GPIO 51 on P9_16. There
...
static unsigned int gpioLED = 48;       ///< hard coding the LED gpio for this example to P9_15 (GPIO48)
static unsigned int gpioButton = 51;   ///< hard coding the button gpio for this example to P9_16 (GPIO51)
...
// gpio_set_value(gpioLED, ledOn);          // Not required as set by line above (here for reference)
   gpio_export(gpioLED, false);             // Causes gpio48 to appear in /sys/class/gpio
			                    // the bool argument prevents the direction from being changed
   gpio_request(gpioButton, "sysfs");       // Set up the gpioButton
   gpio_direction_input(gpioButton);        // Set the button GPIO to be an input
   gpio_set_debounce(gpioButton, 200);      // Debounce the button with a delay of 200ms
   gpio_export(gpioButton, false);          // Causes gpio51 to appear in /sys/class/gpio
			                    // the bool argument prevents the direction from being changed
